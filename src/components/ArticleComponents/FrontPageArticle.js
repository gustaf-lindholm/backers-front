...
const FrontPageArticel = ({ image, introTextId, headerId, children, topBorder }) => {

  const bt = topBorder && "bt";

  return (
    <article className={`${bt} pv4 bb b--black-10 ph3 ph0-l`}>
      <div className="flex flex-column flex-row-ns">
        <div className="w-100 w-60-ns pr3-ns order-2 order-1-ns">
          <h1 className="f3 mt0 lh-title"><FormattedMessage id={headerId} /></h1>
          <p className="f5 f4-l lh-copy">
            {introTextId && <FormattedHTMLMessage id={introTextId} />}
          </p>
          {children}
        </div>
